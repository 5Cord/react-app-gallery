...
function App() {
  ...
  React.useEffect(() => {
    ...
    fetch(`https://65b3799d770d43aba479d6cb.mockapi.io/photo?page=${page}&limit=3&${category}`)
      .then((res) => res.json())
      .then((json) => {
        setCollections(json)
      }).catch((err) => {
  ...
  return (
    <div className="App">
      ...
      <div className="content">
        {isLoading ? (<h2>Идёт загрузка ...</h2>) : (collections.filter(obj => obj.name.toLowerCase().includes(searchValue.toLocaleLowerCase())).map((obj, index) => (
